...
let CrossWrapper = styled(FontAwesomeIcon)`
  font-size: 25px;
  float: right;
`;
...
function SearchContainer (props){
  ...
  const [coordsInner, setCoordsInner] = useState({
    ...
    return (
      <SearchWrapper>
        <CrossWrapper icon={faTimes}
          onClick = {()=>{searchContainerDown()}} />
        <SearchBar setCoords={setCoordsInner} />
        <SearchButton isActive={false} className="search__button"
          onClick={()=>{
            getCoordinates();
            searchContainerDown();
          }}>
        ...
        <MapPointer className="search__mapPointer" onClick={()=>{searchContainerDown()}}>
          <FontAwesomeIcon className="search__lctnIcn search__lctnIcn_arrow"
          icon={faLocationArrow}>   </FontAwesomeIcon>
          <FontAwesomeIcon className="search__lctnIcn search__lctnIcn_marker"
          icon={faMapMarkerAlt}>  </FontAwesomeIcon>
          Выбрать на карте
        </MapPointer>
      </SearchWrapper>
    )
